...
/**
 * I added font customization and changed some colours....
 * 
 * @author Germuth
 * 
 */
public class SwipeyTabs extends ViewGroup implements OnPageChangeListener {
	...
	private int mBottomBarColor = 0xFF0000;
	// text color for all other tabs
	private int mTextColor = 0xFF000000;// 0xff949494;
	// text font for each tab name
	...
	public void setAdapter(SwipeyTabsAdapter adapter) {
		...
		if (mAdapter != null) {
			...
			for (int i = 0; i < count; i++) {
				TextView curr = mAdapter.getTab(i, this);
				if (mTypeface != null) {
					curr.setTypeface(this.mTypeface);
				}
				addView(curr);
			}
	...
	protected void dispatchDraw(Canvas canvas) {
		if (mCurrentPos != -1) {
			...
			// TODO not sure if i want the extra animation...
			// text color can change based on position of tab, might be cool
			...
			for (int i = 0; i < count; i++) {
				final TextView tab = (TextView) getChildAt(i);
				if (mCurrentPos == i) {
					// TODO : not sure why mBottomBarColor was wrong
					tab.setTextColor(interpolateColor(0xFFCC0000,// mBottomBarColor,
